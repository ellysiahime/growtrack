...
export default function ExamPage() {
  ...
  const [selectedExam, setSelectedExam] = useState<ExamPeriod | null>(null);
  ...
  const openEditModal = (exam: ExamPeriod) => {
    setForm({
  ...
  const openDetailModal = (exam: ExamPeriod) => {
    setModalExam(exam);
    setShowModal('detail');
  };
  ...
  const getExamTypeColors = (examType: string) => {
    if (examType === "Midterm") {
      return {
        iconBg: "bg-sky-100",
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-700 text-lg">Manage and track all exam periods</p>
          </div>
          {isOwner && (
            <button
        ...
        {/* Exam Detail/Create/Edit/Delete Modal */}
        {showModal && (
          <div 
          ...
          >
            ...
            >
                ...
                <>
                  ...
                  <div className="space-y-4">
                    <div className="bg-red-50 rounded-2xl p-4">
                      <p className="text-red-700 font-medium">Are you sure you want to delete this exam?</p>
                      <div className="mt-2 text-sm text-gray-600">
                        <p>Year {modalExam.year} • Class {modalExam.class} • Term {modalExam.term}</p>
                        <p className="mt-1">{modalExam.exam_type}</p>
              ...
              {showModal === 'detail' && modalExam && (
                <>
                  <div className="flex items-center justify-between mb-6">
                    <h2 className="text-2xl font-bold text-pink-700">Exam Details</h2>
                  </div>
                  <div className="space-y-4">
                    <div className={`${getExamTypeColors(modalExam.exam_type).modalBg} rounded-2xl p-4`}>
                      <h3 className={`font-bold ${getExamTypeColors(modalExam.exam_type).headerText} mb-2`}>{modalExam.exam_type}</h3>
                      <p className="text-sm text-gray-600">Year {modalExam.year} • Class {modalExam.class} • Term {modalExam.term}</p>
                    </div>
                    <div className="space-y-3">
                      <div className="flex justify-between">
                        <span className="text-gray-600">Dates:</span>
                        <span className="font-semibold">
                          {formatDate(modalExam.start_date)} - {formatDate(modalExam.end_date)}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Teacher:</span>
                        <span className="font-semibold">{modalExam.homeroom_teacher}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">CGPA:</span>
                        <span className="font-semibold">{modalExam.cgpa}</span>
                      </div>
                    </div>
                    <div className="pt-4 border-t border-pink-100">
                      <span className={`inline-block px-3 py-1 rounded-full text-sm font-semibold ${getStatusColor(modalExam.start_date, modalExam.end_date)}`}>
                        {getStatusText(modalExam.start_date, modalExam.end_date)}
                      </span>
                    </div>
                  </div>
                </>
              )}
              {(showModal === 'create' || (showModal === 'edit' && modalExam)) && (
                <>
                  <div className="flex items-center justify-between mb-6">
                  ...
                  <form onSubmit={handleFormSubmit} className="space-y-4">
                    ...
                    {formError && <div className="text-red-600 text-center font-bold text-sm">{formError}</div>}
                    <div className="flex gap-4 justify-end">
                      <button
                        type="button"
                        onClick={closeModal}
                      ...
                      <button
                        ...
                        disabled={formLoading}
                      >
                        {formLoading ? 'Saving...' : showModal === 'create' ? 'Create' : 'Update'}
                      </button>
                    </div>
                  </form>
                </>
